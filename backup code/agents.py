...
class BaseQLearningAgent:
    def __init__(self, n_actions, exploration_rate=1.0, exploration_decay=0.999,
                 min_exploration_rate=0.001, type='egreedy'):
        self.n_actions = n_actions
        self.exploration_rate = exploration_rate
        self.exploration_decay = exploration_decay
        self.min_exploration_rate = min_exploration_rate
        self.type = type
        self.q_table = {}  # key: state
        self.action_counts = {}  # key: state
        self.time = 0
    ...
    def choose_action(self, key):
        ...
        if self.type == 'egreedy':
            return self.egreedy_choice(key)
        elif self.type == 'ucb':
            return self.ucb_choice(key)
        elif self.type == 'softmax':
            return self.softmax_choice(key)
        else:
    ...
    def ucb_choice(self, key):
        ...
        return np.argmax(
            self.q_table[key] + self.exploration_rate *
            np.sqrt(np.log(self.time + 1) / (counts + 1e-5))
        )

    def softmax_choice(self, key, tau=1.0):
        probs = softmax(self.q_table[key] / tau)
