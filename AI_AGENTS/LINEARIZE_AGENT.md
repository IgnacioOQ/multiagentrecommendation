...
status: active
type: how-to
description: Instructions for the Linearize Agent — vectorizing the simulation by replacing Python object loops with NumPy matrix operations, with equivalence and verification requirements.
label: [agent, skill]
injection: directive
volatility: evolving
...
- **Directionality Note:** Recall that if $A \to B$ ($A$ cites $B$), then $B$ observes $A$.
...
Replace the per-agent experiment loop:
...
- [x] Confirm the graph direction logic perfectly.
...
- [x] Log results in `worklog.jsonl`.
